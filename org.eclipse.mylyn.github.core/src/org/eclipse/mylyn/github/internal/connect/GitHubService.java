 ...
 *  
 */
package org.eclipse.mylyn.github.internal.connect;
...
import org.eclipse.mylyn.github.internal.GitHubShowIssue;
import org.eclipse.mylyn.github.internal.model.GitHubComments;
import org.eclipse.mylyn.github.internal.model.GitHubIssue;
import org.eclipse.mylyn.github.internal.model.GitHubIssues;
import org.eclipse.mylyn.github.internal.model.GitHubLabels;
...
public class GitHubService {
    ...
    private final static String ADD_LABEL = "label/add/";
    private final static String REMOVE_LABEL = "label/remove/";
     ...
     */
    public GitHubIssues searchIssues(final String user, final String repo, final String state,
            final String searchTerm) throws GitHubServiceException {
        GitHubIssues issues = null;
        GetMethod method = null;
        try {
            // build HTTP GET method
            if (searchTerm.trim().length() == 0) { // no search term: list all
                method = new GetMethod(gitURLBase + gitIssueRoot + LIST + user + "/" + repo + "/"
                        + state);
            } else {
                method = new GetMethod(gitURLBase + gitIssueRoot + SEARCH + user + "/" + repo + "/"
                        + state + "/" + searchTerm);
            }
     ...
     */
    public boolean addLabel(final String user, final String repo, final String label,
            final int issueNumber, final GitHubCredentials credentials)
            throws GitHubServiceException {
        ...
        try {
            // build HTTP GET method
            method = new PostMethod(gitURLBase + gitIssueRoot + ADD_LABEL + user + "/" + repo + "/"
     ...
     */
    public boolean removeLabel(final String user, final String repo, final String label,
            final int issueNumber, final GitHubCredentials credentials)
            throws GitHubServiceException {
        PostMethod method = null;
        boolean success = false;
        try {
            // build HTTP GET method
            method = new PostMethod(gitURLBase + gitIssueRoot + REMOVE_LABEL + user + "/" + repo
                    + "/" + label + "/" + Integer.toString(issueNumber));
     ...
     */
    public GitHubIssue editIssue(final String user, final String repo, final GitHubIssue issue,
            final GitHubCredentials credentials) throws GitHubServiceException {
        PostMethod method = null;
        try {

            // Create the HTTP POST method
            method = new PostMethod(gitURLBase + gitIssueRoot + EDIT + user + "/" + repo + "/"
                    + issue.getNumber());
            // Set the users login and API token
            final NameValuePair login = new NameValuePair("login", credentials.getUsername());
            final NameValuePair token = new NameValuePair("token", credentials.getApiToken());
    ...
    public GitHubIssue showIssue(final String user, final String repo, final String issueNumber)
            throws GitHubServiceException {
        GetMethod method = null;
        try {
            // build HTTP GET method
            method = new GetMethod(gitURLBase + gitIssueRoot + SHOW + user + "/" + repo + "/"
                    + issueNumber);
    ...
    private GitHubIssue changeIssueStatus(final String user, final String repo,
            String githubOperation, final GitHubIssue issue, final GitHubCredentials credentials)
            throws GitHubServiceException {
        PostMethod method = null;
        try {

            // Create the HTTP POST method
            method = new PostMethod(gitURLBase + gitIssueRoot + githubOperation + user + "/" + repo
                    + "/" + issue.getNumber());
            // Set the users login and API token
            final NameValuePair login = new NameValuePair("login", credentials.getUsername());
            final NameValuePair token = new NameValuePair("token", credentials.getApiToken());
    ...
    public GitHubComments getComments(final String user, final String repo, final String issueNumber)
            throws GitHubServiceException {
        GetMethod method = null;
        GitHubComments comments = null;
        try {
            // build HTTP GET method
            method = new GetMethod(gitURLBase + gitIssueRoot + COMMENTS + user + "/" + repo + "/"
                    + issueNumber);
            // execute HTTP GET method
            executeMethod(method);
            // transform JSON to Java object
    ...
    public GitHubLabels getLabels(final String user, final String repo, final String issueNumber)
            throws GitHubServiceException {
        GetMethod method = null;
        GitHubLabels labels = null;
        try {
            // build HTTP GET method
            method = new GetMethod(gitURLBase + gitIssueRoot + LABELS + user + "/" + repo);
            // execute HTTP GET method
            executeMethod(method);
            // transform JSON to Java object
    ...
    public void addComment(String user, String repo, GitHubIssue issue,
        ...
        try {
            // Create the HTTP POST method
            method = new PostMethod(gitURLBase + gitIssueRoot + ADD_COMMENT + user + "/" + repo
                    + "/" + issue.getNumber());
            // Set the users login and API token
            final NameValuePair login = new NameValuePair("login", credentials.getUsername());
            final NameValuePair token = new NameValuePair("token", credentials.getApiToken());
