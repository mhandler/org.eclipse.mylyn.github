...
import org.eclipse.mylyn.github.internal.GitHubTaskDataHandler;
...
public class GitHubRepositoryConnector extends AbstractRepositoryConnector {
    ...
    public IStatus performQuery(TaskRepository repository, IRepositoryQuery query,
        ...
        String[] stati = getQueryStati(queryStatus);

        monitor.beginTask("Querying repository ...", stati.length);
        try {
            ...
            for (String status : stati) {
                performQueryForStatus(repository, query, collector, monitor, user, project, status);
            }
    ...
    private void performQueryForStatus(TaskRepository repository, IRepositoryQuery query,
            TaskDataCollector collector, IProgressMonitor monitor, String user, String project,
            String status) throws GitHubServiceException {
        GitHubIssues issues = service.searchIssues(user, project, status, query
                .getAttribute("queryText"));

        // collect task data
        for (GitHubIssue issue : issues.getIssues()) {
            TaskData taskData = taskDataHandler.createPartialTaskData(repository, monitor, user,
                    project, issue);
    ...
    private String[] getQueryStati(String queryStatus) {
        String[] statuses;
    ...
    public String computeTaskRepositoryProject(TaskRepository repository) {
    ...
    public String computeTaskRepositoryUser(TaskRepository repository) {
    ...
    public TaskData getTaskData(TaskRepository repository, String taskId, IProgressMonitor monitor)
        ...
        try {
            ...
            TaskData taskData = taskDataHandler.createTaskData(repository, monitor, user, project,
                    issue);
            return taskData;
        } catch (GitHubServiceException e) {
    ...
    private void setProxySetup(TaskRepository repository) {
        boolean useDefaultProxy = Boolean.valueOf(repository
    ...
    public String getRepositoryUrlFromTaskUrl(String taskFullUrl) {
        if (taskFullUrl != null) {
            Matcher matcher = Pattern.compile("(http://.+?)/issues/issue/([^/]+)").matcher(
                    taskFullUrl);
            if (matcher.matches()) {
                return matcher.group(1);
            }
